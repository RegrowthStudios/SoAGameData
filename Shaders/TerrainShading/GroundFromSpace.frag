...
{	
	...
	}else if (textureUnitID == 2.0){ //BIOME MAP
		MaterialDiffuseColor = texture( textures[2], UV ).rgb;
	}
	
	//vec3 tex = 2*texture( normalMap, UV).rgb - vec3(1, 1, 1);
	//vec3 norml = vec3(tex.r, tex.b, tex.g); 
    ...
    color.rgb = 1.0 * (MaterialAmbiantColor 
        + sColor * 2.0
        + lightMod * lightColor * (MaterialDiffuseColor * fragColor * cosTheta + 
