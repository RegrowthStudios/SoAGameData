...
function updateUI(t) 
  if t == 0 then
    ...
    Label.setText(massPanel.label, string.format("Mass: %.2e kg  /  %.4g earth masses", mass, mass / 5.97219e+24))
    ...
    Label.setText(gravityPanel.label, string.format("Surface gravity: %.2f g  /  %.2f m/s2", grav / 9.80665, grav))
    Label.setText(densityPanel.label, string.format("Mean density: %.2f g/cm3", Space.getAverageDensity(t) / 1000))
...
function init()
  ...
  WidgetList.setDimensionsPercentage(widgetList, 0.5, 0.9)
  WidgetList.setMinSize(widgetList, 300, 100)
  WidgetList.setAutoScroll(widgetList, true)
  
  namePanel = newPanel();
  Label.setXPercentage(namePanel.label, 0.25)
